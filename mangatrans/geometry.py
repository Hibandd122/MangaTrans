...
Sửa lỗi v15:
- `_row_extents` Python loop O(h × w_xs) — vectorize bằng `np.split` theo run-length.
- `find_bubble_interior._cache` keyed by `id(image)` → cache poisoning khi numpy GC.
  Đổi sang InteriorCache instance-bound (per-pipeline) hoặc hashable key.
"""
...
def exclude_other_bbox_regions(mask: np.ndarray, bbox, other_bboxes) -> np.ndarray:
    ...
    Distance dùng Chebyshev (max of dx, dy). Equal distance → ưu tiên giữ.
    """
...
class InteriorCache:
    ...
    Trước đây cache là global dict keyed by id(image), bị poison khi GC tái dùng id.
    Giờ instance-bound: pipeline tạo 1 cache cho mỗi ảnh, drop sau khi xong page.
    """
...
def find_bubble_interior(image: np.ndarray, bbox, cfg: GeometryConfig,
    ...
    Chiến lược DARK OUTLINE separator: threshold dark, dilate → barrier giữa
    bubble và gutter → CC riêng. Local search trong ROI mở rộng theo pad.
    Fallback global CC nếu local fail.

    Trả uint8 mask shape (h, w).
    """
    h, w = image.shape[:2]
    ...
    # --- (1) Local CC trong ROI với DARK OUTLINE separator ---
    for pad in (cfg.roi_pad_factor, cfg.roi_pad_factor * 2, cfg.roi_pad_factor * 3):
        ...
        # Grid sample 25 points + outer + center
...
def find_bubble_polygon(image: np.ndarray, bbox, text_seg_mask: np.ndarray,
    ...
    Dùng comic-text-detector seg mask (pixel-level) làm SEED chính xác để pick
    đúng CC chứa text. Voting overlap, không phải grid heuristic.

    Trả dict {mask, contour Nx2, area, centroid} hoặc None.
    """
    h, w = image.shape[:2]
    x1, y1, x2, y2 = clamp_bbox(bbox, w, h)
    bw, bh = x2 - x1, y2 - y1
    ...
    if len(simplified) < 3:
        # Degenerate (collinear) — fallback contour gốc.
...
def largest_inscribed_rect(mask: np.ndarray, center_hint=None) -> Optional[tuple[int, int, int, int]]:
    ...
    Distance transform → tâm xa edge nhất → expand 4 hướng → binary search shrink
    để rect nằm trọn trong mask (kể cả góc bubble cong).
    """
    if not mask.any():
        return None
...
# --------------------------- Row extents (vectorized) --------------------------- #

def row_extents(mask: np.ndarray) -> list[Optional[tuple[int, int]]]:
    """Cho mỗi y trong mask, tìm RUN ngang dài nhất → (xmin, xmax).

    Vectorized: dùng prefix-sum / np.diff thay vì Python loop O(h × w_xs).
    """
    h = mask.shape[0]
    binmask = mask > 0
    out: list[Optional[tuple[int, int]]] = [None] * h

    # Tìm rows có pixel mask
    row_has_pixel = binmask.any(axis=1)
    nonempty_rows = np.where(row_has_pixel)[0]
    if nonempty_rows.size == 0:
        return out

    # Cho mỗi non-empty row, find longest run of trues
    # Sử dụng diff trên column dim để tìm transitions.
    # Tuy vẫn loop theo y, nhưng inner ops là numpy → vẫn nhanh hơn cũ.
...
def polygon_row_extents(contour: np.ndarray, h: int,
    ...
    Mỗi y, intersect polygon với scanline → các edge crossings (pairs in/out).
    Pick segment chứa anchor_x. Tail bubble bị bỏ tự nhiên (không cùng đoạn).
    """
    ...
    # Vectorize per-scanline crossing computation.
    # edges array: (E, 4) [ymin, ymax, xa, xb]
    ...
    for y in range(h):
        ...
        if xs.size < 2:
            continue
        xs.sort()
        # Pair (in,out): pick segment chứa anchor_x; fallback đoạn dài nhất.
        best_seg = None
        best_len = 0.0
        contains = False
