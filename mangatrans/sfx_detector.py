...
Multi-signal classifier:
  - bubble class (detector cls=0 bubble / cls=1 free-text/SFX)
  - bubble shape (round → dialogue, jagged/none → SFX)
  - text length (≤3 char + punctuation → SFX)
  - script content (CJK fullwidth + isolated kana → SFX)
  - font size (huge/stylized → SFX; nhỏ đều → narration box)
  - bbox aspect (rất cao/rất dẹt + cls=1 → action SFX)

Output per block:
  - role: 'dialogue' | 'narration' | 'sfx'
  - subtype (for SFX): 'action' | 'ambient' | 'emotion' | 'unknown'
  - should_translate: bool — gợi ý cho translator
  - should_preserve_pixels: bool — gợi ý preserve nguyên bản (CJK stylized SFX)

Subtype heuristic dựa trên token list. Khi không match → 'unknown'.

KHÔNG dùng ML model — heuristic đủ chính xác và rẻ.
"""
...
from .script_classifier import ScriptProfile, classify_script


# Common action SFX tokens (manga English/JP transliteration)
_ACTION_SFX = {
    # Impact
...
@dataclass
class SFXProfile:
    """Kết quả phân loại 1 block."""

    role: str  # 'dialogue' | 'narration' | 'sfx'
    subtype: str = "unknown"  # cho SFX: action/ambient/emotion
...
class SFXDetectorConfig:
    short_token_max: int = 3                 # text dài <= n char (no space) → candidate SFX
    bubble_class_sfx: int = 1                # cls=1 từ detector = SFX likely
    narration_aspect_max: float = 0.6        # height/width <= n → box ngang dẹt
    narration_min_chars: int = 12            # text dài >= n + bbox dẹt + cls=1 → narration box
    huge_size_ratio: float = 0.025           # bbox area / page area >= 2.5% → likely big SFX
...
class SFXDetector:
    ...
    def classify(self, block: dict, ocr_text: str,
        ...
        """block: dict {'bbox': [...], 'cls': 0/1, 'score': float}.

        `ocr_conf` (optional): nếu truyền, dùng làm tín hiệu "OCR struggled" →
        preserve pixels khi conf thấp + bbox lớn (SFX bị art che).
        """
        ...
        aspect_hw = bh / bw  # >1 vertical, <1 horizontal

        cfg = self.config

        # Rule 1: cls=0 (bubble round) + có letter → dialogue mặc định
        ...
        # Rule 2: cls=1 + bbox dẹt ngang + text dài → narration box
        ...
        # Stylized SFX (huge bbox hoặc CJK pure) → preserve pixels.
        # Big stylized SFX rất khó redraw bằng font thường (Arial Bold) — output
        # trông lạc. CJK pure cũng vậy (kana/han stylized). Trigger thêm: ocr_conf
        # thấp + bbox khá lớn (>=1.5% page) = OCR struggled, có thể SFX bị art che.
        # `sparse_letters`: moderate bbox + < 4 chữ đọc được = SFX fragmented
        # (vd "COUGH" bị đầu nhân vật che → OCR chỉ ra "GH").
        ...
        # Latin SFX ngắn ngoài bubble (cls=1 + ≤3 letters) — preserve pixels
        # mặc định. SFX hand-drawn (TAP, BANG, COUGH…) trông đẹp hơn nhiều khi
        # giữ nguyên art gốc thay vì render lại bằng font thường.
        ...
        # Rule 4: bubble round nhưng text ngắn (interjection) → dialogue SFX hybrid
        ...
        # Default: cls=1 nhưng không match SFX rule → narration
        if cls == cfg.bubble_class_sfx:
            return SFXProfile(
                role="narration",
                should_translate=True,
    ...
    def _subtype(self, text: str, profile: ScriptProfile) -> str:
        """Heuristic match action/ambient/emotion."""
        t = (text or "").strip().lower().strip("?!.,'\"")
        if not t:
            return "unknown"
        # Strip duplicate letters (haaaa → ha) for matching
        compact = "".join(ch for i, ch in enumerate(t)
                          if i == 0 or ch != t[i - 1])

        # Emotion priority — câu cảm thán ngắn
